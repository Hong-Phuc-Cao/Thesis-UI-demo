<template>
    <div class="container">
        ...
        <div class="container__actions">
            ...
            <them-mon-hoc-modal-vue
                :isOpen="hienModalThem"
                @close="dong_modal_them_mon_hoc"
                @save="goi_api_them_mon_hoc"
            />
            <div class="container__table">
                <table border="1">
                    ...
                    <tbody>
                        <tr v-for="(monHoc, index) in danhSachMonHoc" :key="monHoc.MaMon">
                            <td>{{index + 1}}</td>
                            <td>{{monHoc.MaMon}}</td>                 
                            <td>{{monHoc.TenMon}}</td>
                            <td>{{monHoc.TenBM}}</td>
                            <td>
                                <button @click="mo_modal_sua_mon_hoc(monHoc)">✏️</button>
                            </td>
                            <td>
                                <button @click="goi_api_xoa_mon_hoc(monHoc.MaMon)">🗑️</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
                <sua-mon-hoc-modal-vue
                    :isOpen="hienModalSua"
                    :monHoc="monHocDangChinhSua"
                    @close="dong_modal_sua_mon_hoc"
                    @save="goi_api_sua_mon_hoc"
                />
            </div>
        </div>
    </div>
...
import ThemMonHocModalVue from '../../components/admin/them-mon-hoc-modal.vue'
import SuaMonHocModalVue from '../../components/admin/sua-mon-hoc-modal.vue'
...
export default {
    ...
    methods:{
        ...
        async goi_api_them_mon_hoc(monHoc){
            console.log('Môn học  từ modal:')
            console.log(monHoc)
            try{
                const response = await axios.post(`
                    ${API_BASE_URL}/mon-hoc/them-mon-hoc
                `, monHoc)
                if(response.data.success){
                    Swal.fire({
                        icon: 'success',
        ...
        async goi_api_sua_mon_hoc(monHoc){
            const MaMon = monHoc.MaMon
            const data = {
            ...
            try{
                const response = await axios.put(
                    `${API_BASE_URL}/mon-hoc/sua-mon-hoc?MaMon=${MaMon}`,
                data)
                    if (response.data.success) {
                        Swal.fire({
                            icon: 'success',
            ...
            if(result.isConfirmed){
                try{
                    const response = await axios.delete(`
                        ${API_BASE_URL}/mon-hoc/xoa-mon-hoc?MaMon=${id}
                    `)
                    if(response.data.success){
                        Swal.fire({
                            icon: 'success',
        ...
        dong_modal_them_mon_hoc(){
            console.log('Close modal button works')
            this.hienModalThem = false
        },

        mo_modal_sua_mon_hoc(monHoc){
            this.monHocDangChinhSua = {...monHoc}
            this.hienModalSua = true
        },

        dong_modal_sua_mon_hoc(){
            this.hienModalSua = false
        }
    }
