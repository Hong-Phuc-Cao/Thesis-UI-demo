    ...
    <div class="container">
        ...
        <div class="container__action">
            <div class="container__filter">
                ...
                <button id="save-button" @click="luu_phan_cong" v-if="duocXoa">
                    Lưu
                </button>
            </div>
        </div>
    ...
    methods:{
        ...
        async hien_thong_tin(namhoc){
            ...
            await Promise.all([
                ...
                this.hien_danh_sach_phan_cong(namhoc, '1'),
                this.hien_danh_sach_phan_cong(namhoc, '2'),
                this.hien_danh_sach_giao_vien_giu_chuc_vu(namhoc, '1'),
                this.hien_danh_sach_giao_vien_giu_chuc_vu(namhoc, '2'),
                this.hien_danh_sach_giao_vien_kiem_nhiem(namhoc, '1'),
                this.hien_danh_sach_giao_vien_kiem_nhiem(namhoc, '2'),
        ...
        async hien_danh_sach_mon_theo_to_bo_mon(){
            try{
                const response = await axios.get(`${API_BASE_URL}/mon-hoc/danh-sach-mon-hoc-theo-bo-mon?MaBM=${this.MaBM}`)
                if(response.data.success){
        ...
        async hien_danh_sach_giao_vien_theo_bo_mon(){
            try{
                const response = await axios.get(`${API_BASE_URL}/giao-vien/danh-sach-giao-vien-bm?MaBM=${this.MaBM}`)
                if(response.data.success){
        ...
        async hien_danh_sach_phan_cong(namhoc, hocky){
            try{
                const response = await axios.get(`
                    ${API_BASE_URL}/giang-day/danh-sach-giang-day?NamHoc=${namhoc}&HocKy=${hocky}&MaBM=${this.MaBM}
                `)
                if(response.data.success){
        ...
        async hien_danh_sach_giao_vien_giu_chuc_vu(namhoc, hocky){
            //console.log('Giữ chức vụ namhoc - hk:', namhoc, hocky);
            try{
                const response = await axios.get(`
                    ${API_BASE_URL}/giang-day/lay-so-tiet-mien-giu-chuc-vu?NamHoc=${namhoc}&HocKy=${hocky}&MaBM=${this.MaBM}
                `)
                if(response.data.success){
                    ...
                    //console.log('Danh sách gcv:', this.danhSachGiaoVienGiuChucVu);
                }
            }catch(error){
                console.error(error)
                throw error
        ...
        async hien_danh_sach_giao_vien_kiem_nhiem(namhoc, hocky){
            
            try{
                const response = await axios.get(`
                    ${API_BASE_URL}/giang-day/lay-so-tiet-mien-kiem-nhiem?NamHoc=${namhoc}&HocKy=${hocky}&MaBM=${this.MaBM}
                `)
                if(response.data.success){
                    const data = response.data.data.map(item => ({
                        ...item,
                    ...
                    //console.log('Danh sách GV KN: ', this.danhSachGiaoVienKiemNhiem);
                }
            }catch(error){
                console.error(error)
                throw error
        ...
        showPopup(magv, hocky, noidung) {
            if(noidung == 'ChucVu'){
                ...
                const danhSachChucVu = giaovien.length > 0
                                        ? giaovien.map(gv => `${gv.TenCV} (${gv.SoTietMien || 0} tiết)`).join(`<br>`)
                                        : 'Không có chức vụ'
                    ...
                    html: `
                        <ul class="custom-list">
                            <li><strong>Mã giáo viên:</strong> ${magv}</li>
                             <li><strong>Danh sách chức vụ:</strong> <br/> ${danhSachChucVu}</li>
                ...
                const danhSachKiemNhiem = giaovien.length > 0
                                        ? giaovien.map(gv => `${gv.TenNV} (${gv.SoTietMien || 0} tiết`).join(`<br>`)
                                        : 'Không có kiêm nhiệm'
                const soTietMien = giaovien.reduce((total, gv) => total + (gv.SoTietMien || 0), 0)
                ...
                Swal.fire({
                    ...
                    html: `
                        <ul class="custom-list">
                            <li><strong>Mã giáo viên:</strong> ${magv}</li>
                            <li><strong>Danh sách kiêm nhiệm:</strong> <br/> ${danhSachKiemNhiem}</li>
            ...
            }else console.log('Có lỗi hiển thị thông tin bảng kết quả');
        },
            ...
            try {
                ...
                for (const hocKy of semesters) {
                    ...
                    for (const pc of phanCongSua) {
                        const { Khoi, KyHieu, STTLop } = this.tachTenLop(pc.lop.TenLop);
                        await axios.put(
                            `${API_BASE_URL}/giang-day/sua-giang-day?MaMon=${pc.mon.MaMon}&STTLop=${STTLop}&Khoi=${Khoi}&KyHieu=${KyHieu}&NamHoc=${this.NamHoc}&HocKy=${hocKy}`,
                            {
                                MaGV: pc.maGV,
                            },
                            { withCredentials: true }
                    ...
                    for (const pc of phanCongXoa) {
                        const { Khoi, KyHieu, STTLop } = this.tachTenLop(pc.lop.TenLop);
                        await axios.delete(
                            `${API_BASE_URL}/giang-day/xoa-giang-day?MaMon=${pc.mon.MaMon}&STTLop=${STTLop}&Khoi=${Khoi}&KyHieu=${KyHieu}&NamHoc=${this.NamHoc}&HocKy=${hocKy}`,
                            { withCredentials: true }
                        );
                    }
                }
